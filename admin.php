	...
	<style>
		...
		* {
		...
		body {
			...
			display: flex;
			background-color: #f4f4f9; /* Background matches dashboard */
			height: 100vh;
			overflow: hidden;
		}
...
<body>
	...
	<div class="content">
		...
		<table>
			...
			<tbody>
				<?php	
					$sql = "SELECT * FROM file ORDER BY date_added DESC, time DESC;";
					...
					if ($result->num_rows > 0) {
						while ($row = $result->fetch_assoc()) {
							...
							<tr>
								...
								<td><?php echo htmlspecialchars($time); ?> <?php echo htmlspecialchars($date_added); ?></td>
								<td>
									<a class="table-link" href="<?php echo ($type === 'folder') 
										? 'folder.php?folder=' . urlencode($name) 
										: 'viewFile.php?file=' . urlencode($file); ?>" 
										target="_blank" 
